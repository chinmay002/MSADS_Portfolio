...
cost_threshold = 12500
hmo$expensive <- hmo$cost
hmo<-mutate(hmo, expensive = ifelse(cost > cost_threshold, "TRUE", "FALSE"))
hmo<-hmo[complete.cases(hmo),]
#Filling missing values using median for hypertension
hmo <- data.frame(expensive=as.factor(hmo$expensive),
                  age = hmo$age, bmi = hmo$bmi, children=hmo$children, smoker=hmo$smoker,
                  exercise = hmo$exercise, hypertension=hmo$hypertension, yearly_physical = hmo$yearly_physical
              )

#DATA PARTITION
library(caret)
set.seed(111)
trainList <- createDataPartition(y=hmo$expensive,p=.75,list=FALSE)
trainset <- hmo[trainList,]
testset <- hmo[-trainList,]
#MODEL
library(kernlab)
final_model <- ksvm(expensive ~ age+bmi+children+smoker+exercise+hypertension+yearly_physical,data=trainset,C=4, rob.model = T, cross = 3)
